...
public class LinkedBlockingQueue<E> extends AbstractQueue<E>
    ...
    public LinkedBlockingQueue(int capacity) {
        ...
        last = head = new Node<E>(null); // head节点是虚节点 count元素计数不包括head节点
    }
    ...
    public void put(E e) throws InterruptedException {
        ...
        putLock.lockInterruptibly(); // 必须要获取到写锁才能进行put操作
        try {
            /*
             ...
             */
            while (count.get() == this.capacity) { // 队列满了
                notFull.await(); // 阻塞在条件队列 等待队列notFull条件满足
            }
            this.enqueue(node); // 入队
            c = count.getAndIncrement(); // 元素入队成功 count计数+1 c为这次入队之前的元素数量
            if (c + 1 < this.capacity) // 这次入队之后队列还没满 可以继续写 就唤醒那些等待在notFull条件的写线程
                notFull.signal();
        } finally {
            putLock.unlock();
        }
        if (c == 0) // 入队之前元素数量0 说明之前是空队列 必然有读线程因为空导致阻塞在notEmpty条件上 现在至少已经有一个元素写入成功了 就可以唤醒notEmpty上面阻塞的读线程
            signalNotEmpty();
    }
    ...
    public E take() throws InterruptedException {
        ...
        final AtomicInteger count = this.count;
        final ReentrantLock takeLock = this.takeLock;
        takeLock.lockInterruptibly(); // 获取到读锁
        try {
            while (count.get() == 0) { // 队列为空 就将当前读线程阻塞在notEmpty条件上
                notEmpty.await();
            }
            x = this.dequeue(); // 出队
            c = count.getAndDecrement(); // count减1 c指的是count减1之前的旧值
            if (c > 1) // 这次出队之前至少2个元素 现在消费了一个 也就是说这次出队之后至少还有一个元素 就可以继续读 唤醒阻塞在notEmpty条件上等待读的线程
                notEmpty.signal();
        } finally {
            takeLock.unlock();
        }
        if (c == capacity) // 出队之前队列已经满了 现在消费了一个 队列就空出来可以写了 唤醒那些阻塞在notFull上等待写的线程
            signalNotFull();
        return x;
    }
