...
public class ReentrantLock implements Lock, java.io.Serializable {
    ...
    abstract static class Sync extends AbstractQueuedSynchronizer {
        ...
        protected final boolean tryRelease(int releases) {
            int c = getState() - releases; // state值
            if (Thread.currentThread() != super.getExclusiveOwnerThread()) // 释放锁的线程必须得是上锁的那个线程
                throw new IllegalMonitorStateException();
            boolean free = false;
            if (c == 0) { // 可重入锁 state的值>1 state=0意味着无锁状态
                free = true;
                this.setExclusiveOwnerThread(null); // 清空占锁线程
            }
            super.setState(c); // state更新
            return free;
        }
    ...
    public ReentrantLock() {
        this.sync = new NonfairSync(); // 默认实现是非公平锁
    }
