...
public class LockSupport {
     ...
     * @since 1.6
     */
    public static void park(Object blocker) { // 阻塞的调用park方法的当前线程
        Thread t = Thread.currentThread(); // 获取调用park方法的当前线程
        setBlocker(t, blocker); // 记录阻塞原因
        U.park(false, 0L); // 利用反射执行park
        setBlocker(t, null); // 执行到该处说明线程恢复 移除阻塞原因(被park的线程恢复原因有3 其他线程调用了unpark方法 该线程被中断 该线程发生了不可预料的事情)
    }
