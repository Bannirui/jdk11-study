    ...
    extends SelectableChannel
{
    ...
    //
    private SelectionKey[] keys = null; // 集合中的元素代表的就是注册在复用器上的channel
    ...
    private void addKey(SelectionKey k) { // 把注册复用器成功的channel缓存起来 底层用的数组 所以设计到数据的扩容
    ...
    private SelectionKey findKey(Selector sel) { // 注册在复用器上的channel都被缓存在keys集合中 根据复用器 找到注册在这个复用器上的channel
        assert Thread.holdsLock(keyLock);
        if (this.keys == null)
            return null;
